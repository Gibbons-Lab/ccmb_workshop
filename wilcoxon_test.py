...
## Input arguments
parser = argparse.ArgumentParser(description='run rank sums tests on case-control dataset')
parser.add_argument('-i', help='input OTU table qiime artifact (rows = samples, '
    + ' columns = phylotypes; default format = tab-delimited)', required=True)
parser.add_argument('-m', help='case-control metadata with "disease_state" column', required=True)
...
parser.add_argument('-t', help='occurence threshold in case or control', default=0.3)
